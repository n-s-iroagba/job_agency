...
export default function ConditionsManagementPage() {
    ...
    if (isLoading) return <div className="p-12 text-center text-[10px] font-bold uppercase tracking-widest text-slate-400">Loading Conditions...</div>;
    ...
    return (
        <div className="font-sans">
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Conditions</h1>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Manage standard job prerequisites</p>
                </div>
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {conditionsList.map((cond: JobCondition) => (
                                <tr key={cond.id} className="hover:bg-slate-50/50 transition-colors group">
                                    ...
                                    <td className="px-6 py-5">
                                        <p className="text-[11px] text-slate-500 line-clamp-1 max-w-sm">{cond.description}</p>
                                    </td>
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex justify-end gap-3">
                                            <Link href={`/admin/conditions/${cond.id}`} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-slate-900 transition-colors">
                                                View
                                            </Link>
                                            <Link href={`/admin/conditions/${cond.id}/edit`} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-blue-600 transition-colors">
                                                Edit
                                            </Link>
                                            <button onClick={() => handleDelete(cond.id)} className="text-[10px] font-bold text-slate-400 uppercase tracking-widest hover:text-red-500 transition-colors">
                                                Delete
                                            </button>
                                        </div>
                                    </td>
                            ...
                            {conditionsList.length === 0 && (
                                <tr>
                                    <td colSpan={3} className="px-6 py-12 text-center">
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-slate-300">No conditions found</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
