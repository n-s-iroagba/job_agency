...
export function ApplicantSidebar({
    ...
    return (
        <>
            ...
            <aside
                className={`h-screen fixed left-0 top-0 bg-white flex flex-col transition-all duration-300 z-[70] border-r border-slate-100 font-sans text-slate-900 overflow-hidden
                    ${isMobileOpen ? 'translate-x-0 w-64' : '-translate-x-full lg:translate-x-0'}
                    ${isCollapsed ? 'lg:w-28' : 'lg:w-64'}
                `}
            >
                {/* Brand / Toggle Integration */}
                <div className={`py-10 border-b border-slate-50 relative flex items-center ${isCollapsed ? 'justify-center' : 'px-8'}`}>
                    <Link href="/" className={`${isCollapsed ? 'hidden' : 'flex flex-col'}`}>
                        ...
                        <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Applicant</span>
                    </Link>
                    {isCollapsed && (
                        <div className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center text-white font-black text-xl italic shadow-lg shadow-slate-900/10">J</div>
                    )}
                    
                    {/* Desktop Toggle Button */}
                    <button
                        onClick={onToggle}
                        className="hidden lg:flex absolute -right-3 top-1/2 -translate-y-1/2 w-6 h-6 bg-white border border-slate-200 rounded-full items-center justify-center text-slate-400 hover:text-slate-900 shadow-sm transition-all"
                    >
                    ...
                    <button
                        onClick={onMobileClose}
                        className="lg:hidden absolute right-4 top-1/2 -translate-y-1/2 text-slate-400"
                    >
                ...
                {/* Profile & Logout */}
                <div className={`p-4 border-t border-slate-50 ${isCollapsed ? 'flex flex-col items-center gap-4' : ''}`}>
                    <div className={`flex items-center bg-slate-50 rounded-2xl border border-slate-100/50 transition-all ${isCollapsed ? 'w-12 h-12 justify-center p-0' : 'gap-4 p-4 mb-4'}`}>
                        <div className={`rounded-xl bg-slate-900 text-white flex items-center justify-center font-bold overflow-hidden ${isCollapsed ? 'w-10 h-10 text-xs' : 'w-10 h-10 text-sm'}`}>
                            {initials}
                        </div>
                        {!isCollapsed && (
                    ...
                    <button
                        ...
                        className={`flex items-center justify-center bg-white border border-slate-200 text-slate-400 hover:text-red-600 hover:border-red-100 hover:bg-red-50 rounded-xl transition-all duration-200 group ${isCollapsed ? 'w-12 h-12' : 'w-full py-4 px-4 gap-2'}`}
